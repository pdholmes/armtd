...
elseif dim == 3
    ...
    C = (C./sqrt(sum(C.^2)))';
elseif (dim >= 4 && dim <= 7)
    % PATRICK: created cross_product_functions for dim >= 4 and <= 7
    func = str2func(sprintf('dim_%s_cross', num2str(dim)));
...
%             v=ndimCross(Q);
    ...
    error('No functions defined for cross products that large...')
end
