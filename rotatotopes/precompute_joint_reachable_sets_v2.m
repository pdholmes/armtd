function [] = precompute_joint_reachable_sets_v2()
...
[dyn_zero_to_t_plan, dyn_t_plan_to_t_total] = ...
    generate_parameterized_dynamics_v2(t_plan, t_total);
...
if ~exist('rotatotopes/joint_reachable_sets_v2', 'dir')
    mkdir('rotatotopes/joint_reachable_sets_v2');
end

% save vector of initial velocity subinterval centers
save('rotatotopes/joint_reachable_sets_v2/c_kvi.mat', 'c_kvi');
...
for j = 1:n_JRS % compute JRS for each velocity subinterval
    ...
    options.x0 = [1; 0; c_kvi(j); c_kai; c_kvi(j); 0]; % start at q_i = 0, so cos(q_i) = 1 and sin(q_i) = 0
    % use two generators, one for K^a_i and one for K^v_i (eq. 8)
    options.R0 = zonotope([options.x0, [0; 0; 0; delta_kai; 0; 0], [0; 0; delta_kvi; 0; delta_kvi; 0]]);
    ...
    filename = sprintf('rotatotopes/joint_reachable_sets_v2/JRS_%0.3f.mat', current_c_kvi);
