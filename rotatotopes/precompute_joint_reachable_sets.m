function [] = precompute_joint_reachable_sets()
...
if ~exist('rotatotopes/joint_reachable_sets', 'dir')
    mkdir('rotatotopes/joint_reachable_sets');
end

% save vector of initial velocity subinterval centers
save('rotatotopes/joint_reachable_sets/c_kvi.mat', 'c_kvi');
...
for j = 1:n_JRS % compute JRS for each velocity subinterval
    ...
    options.x0 = [1; 0; c_kai; c_kvi(j); 0]; % start at q_i = 0, so cos(q_i) = 1 and sin(q_i) = 0
    % use two generators, one for K^a_i and one for K^v_i (eq. 8)
    options.R0 = zonotope([options.x0, [0; 0; delta_kai; 0; 0], [0; 0; 0; delta_kvi; 0]]);
    ...
    t_plan_slice = zonotope_slice(JRS_zero_to_t_plan{end}{1}, 5, t_plan);
    ...
    filename = sprintf('rotatotopes/joint_reachable_sets/JRS_%0.3f.mat', current_c_kvi);
